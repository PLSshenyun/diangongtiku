...
# 导入题库文件
chooseQuestionFile = open("./questions/judge.csv" , "r")
...
for chooseQuestionFileInputLine in chooseQuestionFileInput:
    ...
    chooseQuestionDict[CQFILL[0]] = [CQFILL[0],CQFILL[1],CQFILL[2]]
...
for chooseAnswerListLine in chooseAnswerList:
    ...
    answerHistoryline = [CQD[CALL[0]][0],CQD[CALL[0]][1],CQD[CALL[0]][2],CALL[1],CALL[2]]
    answerHistory.append(answerHistoryline)

# 导出自己的答题记录
import time
name = time.ctime().replace(" ","-").replace(":","-") 
historyOutput = open("./history/{}-judge.csv".format(name) , "w")
historyOutput.write("题号,题目,正确答案,你的答案,结果\n")
